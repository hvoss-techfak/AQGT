...
class VisualizeCvClass:
    ...
    def visualizeOneFrame(self, pose, traj_out, videoFrame, keypoint, scores=None, score_threshold=0.1,
        ...
        if len(self.leftHandStiffnessHistory) == 20:
            lchange = "Left Stiffness change:  {0:+.5f}".format(savgol_filter(np.asarray(self.leftHandStiffnessHistory).flatten(),window_length=7,polyorder=3,deriv=1).flatten().mean())
            rchange = "Right Stiffness change: {0:+.5f}".format(savgol_filter(np.asarray(self.leftHandStiffnessHistory).flatten(), window_length=7, polyorder=3,deriv=1).flatten().mean())
