...
class DetectionLoader_single():
    ...
    def frame_preprocess(self,frame,ie):
        ...
        return imgs, orig_imgs, im_names, im_dim_list

    def image_detection(self,bundle):
        imgs, orig_imgs, im_names, im_dim_list = bundle
        with torch.no_grad():
            # pad useless images to fill a batch, else there will be a bug
            for pad_i in range(1 - len(imgs)):
        ...
        return orig_imgs[0], im_names[0], None, None, None, None, None

    def post_process(self,bundle):
        with torch.no_grad():
            (orig_img, im_name, boxes, scores, ids, inps, cropped_boxes) = bundle
            if orig_img is None:
                return None, None, None, None, None, None, None
            if boxes is None or boxes.nelement() == 0:
                return None, orig_img, im_name, boxes, scores, ids, None
    ...
    def __next__(self):
        ...
        if not grabbed:
            raise StopIteration
        bundle = self.frame_preprocess(frame,self.ie)
        self.ie += 1
        bundle = self.image_detection(bundle)
        bundle = self.post_process(bundle)
        return bundle

class DetectionLoader_own():
    def __init__(self, input_source, detector, cfg, opt, mode='image', batchSize=1, queueSize=128):
        self.cfg = cfg
        self.opt = opt
        self.mode = mode
        ...
        if mode == 'image':
            ...
            assert stream.isOpened(), 'Cannot capture source'
            self.path = input_source
            print("fetching video size. Please wait...")
            self.datalen = self.getDatalen(input_source)
            print("done. Length of video:",self.datalen)
            self.fourcc = int(stream.get(cv2.CAP_PROP_FOURCC))
            self.fps = stream.get(cv2.CAP_PROP_FPS)
            self.frameSize = (int(stream.get(cv2.CAP_PROP_FRAME_WIDTH)), int(stream.get(cv2.CAP_PROP_FRAME_HEIGHT)))
            self.videoinfo = {'fourcc': self.fourcc, 'fps': self.fps, 'frameSize': self.frameSize}
    ...
    def getDatalen(self, input_source):
        import numpy as np
        import cv2
