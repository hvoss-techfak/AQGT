...
def eval_embed(pre_poses, target_poses, net):
    poses_feat, pose_mu, pose_logvar, out_poses = net(pre_poses, target_poses, False)

    recon_loss = F.l1_loss(out_poses, torch.cat((pre_poses, target_poses), dim=1), reduction='none')
    recon_loss = torch.mean(recon_loss, dim=(1, 2))
    loss = torch.mean(recon_loss)

    return loss, out_poses
