...
def train_iter_gan(args, epoch, time_seq, in_text, in_audio, target_poses, vid_indices,
    ...
    if epoch > warm_up_epochs and args.loss_gan_weight > 0.0:
        ...
        out_dir_vec, beat_out, *_ = pose_decoder(pre_seq, time_seq, in_text, in_audio,
                                                 vid_indices)  # out shape (batch x seq x dim)
    ...
    out_dir_vec_rand_vid, out_beat_vec_rand_vid, z_rand_vid, _, _ = pose_decoder(pre_seq, time_seq, in_text, in_audio,
                                                                                 rand_vids)
    beta = 0.05
