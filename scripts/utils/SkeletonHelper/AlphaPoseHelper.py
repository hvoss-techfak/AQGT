...
args.cfg = "configs/halpe_136/resnet/256x192_res50_lr1e-3_2x-regression.yaml"
args.detbatch = 5
args.posebatch = 60
args.detector = "yolo"
args.sp = False
...
class AlphaPoseHelper:
    def __init__(self, inputFile=None, justLoad=False, pose_track=True):
        ...
        if not justLoad:
            if inputFile is None:
                self.det_loader = WebCamDetectionLoader(0, get_detector(args), cfg, args)
                self.frames = -1
            else:
                self.det_loader = DetectionLoader(inputFile, get_detector(args), cfg, args, batchSize=args.detbatch,
                                                  mode='video', queueSize=args.qsize)
                self.frames = self.det_loader.datalen
    ...
    def doOneFrameHeat(self):
        with torch.no_grad():
            (inps, orig_img, im_name, boxes, scores, ids, cropped_boxes) = self.det_loader.pose_queue.get(timeout=5)
            if boxes is None:
            ...
            if args.pose_track:
                boxes, scores, ids, hm, cropped_boxes = track(self.tracker, args, orig_img, inps, boxes, hm,
                                                              cropped_boxes,
                                                              im_name, scores)
            hm = hm.cpu()
    ...
    def doOneFrameFull(self):
        boxes, scores, ids, hm, cropped_boxes, orig_img, im_name = self.doOneFrameHeat()
...
class RealtimeSequence:
        ...
        if ids is not None:
            ...
            for i, id in enumerate(ids):
                ...
                if self.ie - person["last_seen"] > 1:
                    person = self.interpolatePosition(person, pose_coords[i], pose_scores[i],
                                                      self.ie - person["last_seen"])
                    person = self.interpolate3DPosition(person, halpeToCocoSkeleton(pose_coords[i]), pose_scores[i],
                                                        self.ie - person["last_seen"])
    ...
    def createReturnDict(self, person, withImage=True):
        ...
        if self.calculateTrajectory:
            ret["current_3d_pose"] = person["predicted_3d_pos"][-1] / [self.image_dimensions[1],
                                                                       self.image_dimensions[0], 1]

            hand_l_z = person["3d_hand_z_left"][-1][:, 2].reshape((21, 1)) if len(
                person["3d_hand_z_left"]) > 0 else np.zeros((21, 1))
            hand_r_z = person["3d_hand_z_right"][-1][:, 2].reshape((21, 1)) if len(
                person["3d_hand_z_right"]) > 0 else np.zeros((21, 1))

            hand_l_z = hand_l_z + ret["current_3d_pose"][13, 2]
            hand_r_z = hand_r_z + ret["current_3d_pose"][16, 2]

            pos = person["pose_coords"][-1]
            right_hand = np.concatenate((pos[115:136], hand_r_z), axis=1) / [self.image_dimensions[1],
                                                                             self.image_dimensions[0], 1]
            left_hand = np.concatenate((pos[94:115], hand_l_z), axis=1) / [self.image_dimensions[1],
                                                                           self.image_dimensions[0], 1]
    ...
    def interpolate_points(self, p1, p2, n_steps=3):
        """Helper function that calculates the interpolation between two points.txt"""
    ...
    def calculateTraj(self, person, orig_img):
        predicted_3d_pos, predicted_3d_traj = self.aH.do2Dto3D(np.asarray(person["3d_pose_coords"]), orig_img.shape[1],
                                                               orig_img.shape[0])
        ...
        predicted_3d_pos = np.concatenate((person["3d_pose_coords"][-1], predicted_3d_pos[:, 2].reshape((17, 1))),
                                          axis=1)
