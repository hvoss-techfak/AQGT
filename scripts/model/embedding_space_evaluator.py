...
#Embedding space evaluator from: https://github.com/ai4r/Gesture-Generation-from-Trimodal-Context
class EmbeddingSpaceEvaluator(LightningModule):
    def __init__(self, args, embed_net_path, lang_model, device):
        super().__init__()
        self.n_pre_poses = args.n_pre_poses

        # init embed net
        print(embed_net_path)
        ...
        #TODO REPLACE
        print("loading embed net evaluator")
    ...
    def get_diversity_scores(self):
        feat1 = np.vstack(self.generated_feat_list)
        ret = []
        for i in range(100):
            random_idx = torch.randperm(len(self.generated_feat_list))
            ...
            ret.append(feat_dist)
        feat_dist = np.mean(ret)
        return feat_dist

    def get_ground_diversity_scores(self):
        feat1 = np.vstack(self.real_feat_list)
        ret = []
        for i in range(100):
            random_idx = torch.randperm(len(self.real_feat_list))
            shuffle_list = [self.real_feat_list[x] for x in random_idx]
            feat2 = np.vstack(shuffle_list)
            # dists = []
            # for i in range(feat1.shape[0]):
            #     d = np.sum(np.absolute(feat1[i] - feat2[i]))  # MAE
